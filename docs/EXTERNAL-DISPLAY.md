...
For KDE's "Querying input devices failed" message, check whether `/data/local/chroot-distro/ubuntu26/dev/input` exists and whether the Mouse KCM plugin is installed. On the reference session, the plugin exists but `/dev/input` does not; Android routes the USB HID devices to Anland. Do not bind the raw Android input nodes into the chroot during a live session without an isolated test, a touchscreen/ADB recovery path and a verified rollback, because KWin may then see duplicate events.
